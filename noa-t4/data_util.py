...
from Model import NUM_CLASSES, IMAGE_CHANNELS, IMAGE_HEIGHT, IMAGE_WIDTH
...
def dataset_describe(dataset, class_list, logger):
    logger.addline(f"Total size: {len(dataset)}")
    logger.addline(f"Classes: {class_list}")
    logger.addline(f"Number of classes: {len(class_list)}")

    for i in range(len(class_list)):
        count = dataset.targets.count(i)
        percentage = count * 100 / len(dataset)
        logger.addline(f"Size of '{class_list[i]}': {count} | {percentage:.2f}%")
    logger.flush()

def dataset_split_describe(dataset_split, class_list, logger):
    ...
    for i in range(len(class_list)):
        logger.addline_(f"Size of '{class_list[i]}': {count_per_class_train[i]} | {count_per_class_train[i]/len(dataset_split) * 100:.2f}%")

def load_image_folder(root, 
                              transform=None,
                              logger=Logger()):
    
    dataset = torchvision.datasets.ImageFolder(root=root, 
                                               transform=transform)
    logger.addline(f"Image folder: {root}")
    assert NUM_CLASSES == len(dataset.classes), f"Expecting num of classes {NUM_CLASSES}, but ImageFolder found {len(dataset.classes)}"
    dataset_describe(dataset=dataset, class_list=dataset.classes, logger=logger)

    logger.flush()
    logger.describe()
    return dataset

def split_dataset(dataset, train_eval_split=0.7, logger=Logger()):
    ...
    logger.addline_(f"Train-eval split factor: {train_eval_split}")
    logger.addline_(f"Train datapoints: {len(train_ds)}")
    logger.addline_(f"Eval datapoints: {len(eval_ds)}")

    logger.addline_("=== Train Split ===")
    dataset_split_describe(dataset_split=train_ds, class_list=dataset.classes, logger=logger)

    logger.addline_("=== Eval Split ===")
    dataset_split_describe(dataset_split=eval_ds, class_list=dataset.classes, logger=logger)
